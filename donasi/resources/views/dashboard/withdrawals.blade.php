                        ...
                        <tbody>
                            @foreach ($withdrawals as $row)
                                <tr>
                                    ...
                                    <td>
                                        {{ date('d/m/Y H:i', strtotime($row->campaignfunditem->transaction_time)) }}
                                    </td>
                                    <td>
                                        <a href="javascript:void(0)" class="editStatus" data-id="{{ $row->id }}"
                                            data-idfund="{{ $row->campaignfunditem->id }}" data-wallet="campaign_fund"
                                            data-bs-toggle="modal" data-bs-target="#statusModal">
                                            {{ $row->campaignfunditem->no_transaksi }}
                                        </a>
                                    </td>
                                    <td>
                                        {{ $row->funditem->user->name ?? $row->campaignfunditem->user->name }}
                                    </td>
                                    <td>
                                        {{ $row->campaignfunditem->campaign->title ?? '-' }}
                                    </td>
                                    <td>
                                        @if ($row->funditem_id != '')
                                            {{ $row->funditem->transaction_type }} Pribadi
                                        @else
                                            {{ $row->campaignfunditem->transaction_type }} Kampanye
                                        @endif
                                    </td>
                                    <td>
                                        @if ($row->funditem_id != '')
                                            @currency($row->funditem->gross_amount)
                                        @else
                                            @currency($row->campaignfunditem->gross_amount)
                                        @endif
                                    </td>
                                    <td>
                                        @if ($row->funditem_id != '')
                                            {{ $row->funditem->transaction_status }}
                                        @else
                                            {{ $row->campaignfunditem->transaction_status }}
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
...
@section('script')
    <script>
        $('.editStatus').on('click ', function() {
            ...
            $('.modal form').prop('action', '/dashboard/withdrawals/' + id);
