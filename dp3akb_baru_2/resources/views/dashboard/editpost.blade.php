                ...
                <div class="card-body">
                    ...
                    <form action="/dashboard/posts/{{ $post->id }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control mt-1 @error('title') is-invalid @enderror" name="title" type="text" id="title" value="{{ old('title', $post->title) }}" />
                            @error('title')
                        ...
                        <div class="row">
                            <div class="col-md-2 mb-3">
                                <img src="{{ $post->image ? '/storage/' . $post->image : '/assets/img/noimage.jpeg' }}" class="img-thumbnail imagePreview" width="100px">
                            </div>
                            <div class="col-md-10 mb-3">
                                <div class="form-group">
                                    <label for="image">Gambar</label>
                                    <div class="input-group">
                                        <input type="file" name="image" id="image" class="form-control mt-1 @error('image') is-invalid @enderror" onchange="previewImage('image', 'imagePreview')">
                                        @error('image')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="postcategory_id" id="postcategory_id" class="form-control mt-1 @error('postcategory_id') is-invalid @enderror">
                                <option value="">:: Pilih ::</option>
                                @foreach ($categories as $row)
                                <option value="{{ $row->id }}" {{ $row->id == $post->postcategory_id ? 'selected' : '' }}>{{ $row->name }}
                                </option>
                                @endforeach
                            </select>
