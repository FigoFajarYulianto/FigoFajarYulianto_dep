...
class UjianController extends Controller
{
    ...
    public function selesai(Request $request)
    {
        ...
        $nilai_siswa = $this->_hitungNilai($ujian->id);

        $kriteria = Ujian::find($ujian->ujian_id);
        $kandidat = Kandidat::where('siswa_id', $ujian->siswa_id)->first();

        $nilai['ujian_siswa_id'] = $ujian->id;
        $nilai['kriteria_id'] = $kriteria->kriteria_id;
        $nilai['kandidat_id'] = $kandidat->id;
        $nilai['nilai'] = $nilai_siswa;
        $nilai_store = Nilai::create($nilai);

        $nilai['id'] = $nilai_store->id;
        RekapNilai::create($nilai);
